...
流程:
    ...
    6. 调用 Logo_Compliance_Check 进行合规间距/比例检测 → 输出违规报告
    7. 调用 Background_Color_Verify 进行背景颜色校验 → 背景掩码 + K-Means + MLLM分析
    8. 调用 Logo_Font_Verify 进行字体校验 → 并发对比中文/英文全称/英文简称字体样例图
...
from Logo_Compliance_Check import check_compliance, print_report as print_compliance_report, save_gaps_image
...
def run_logo_recognition(
    ...
    返回:
        dict: {
            ...
            "compliance_verification": dict, # 合规间距/比例检测结果
    ...
    if roi_result.get("success") and roi_result.get("best_logo"):
        ...
        compliance_result = check_compliance(
            ...
            img_bgr=img_bgr,
            all_tuples=text_tuples,
        )
        timing["compliance_check"] = time.perf_counter() - t

        if compliance_result.get("passed") is not None:
            status = "PASS" if compliance_result.get("passed") else "FAIL"
            v_count = len(compliance_result.get("violations", []))
            print(f"\n[合规检测] A={compliance_result.get('A', 0):.0f}px  {status}"
                  f"{'  违规' + str(v_count) + '项' if v_count else ''}")

            # 保存合规间距参考图（使用 ROI 切片，与其他图片一致）
            roi_dir = output_dir / input_file.stem
            roi_dir.mkdir(parents=True, exist_ok=True)
            compliance_img_path = str(roi_dir / f"{input_file.stem}_compliance.png")
            # 切换到 ROI 图像坐标系
            if roi_result.get("roi_image") is not None:
                cr = dict(compliance_result)
                ox, oy = roi_result["max_box"][0], roi_result["max_box"][1]
                cr["_img"] = roi_result["roi_image"]
                for k in list(cr.keys()):
                    if k.startswith("_") and k.endswith("_bbox") and cr[k] is not None:
                        b = cr[k]
                        cr[k] = (b[0] - ox, b[1] - oy, b[2] - ox, b[3] - oy)
                cr["A"] = cr.get("A", 1)  # A 值与尺度无关
                save_gaps_image(cr, compliance_img_path)
            else:
                compliance_result["file"] = str(input_file.name)
                save_gaps_image(compliance_result, compliance_img_path)
        else:
            print(f"\n[合规检测] 跳过: {compliance_result.get('error', '')}")
    else:
    ...
    # Step 8: 字体校验（使用 ROI 图像，并发对比中文/英文全称/英文简称字体样例图）
    font_result = None
